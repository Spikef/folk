...
Folk的主题文件放置于`vision/themes`目录下。
...
当type为dynamic时，source提供下拉菜单选项。dynamic表示该参数的可选值由函数动态生成，source即为该匿名函数的源码。
...
* [helper.setUrl](#helper.setUrl)
* [helper.fns](#helper.fns)
* [helper.hook](#helper.hook)
...
fns模块提供了一些辅助函数方法，这里只介绍几个常用的。

**fns.extend(source, target)**

将target对象(json)的元素拷贝至source对象(json)，并返回新的source对象。

**fns.deepClone(source, target)**
...
**fns.isType(object, type)**

相当于`fns.getType(object) === type`。
...
**fns.numberFormat(number, format)**
...
* article.art_tags: 返回文章标签列表，Array类型

文章标签包含以下字段:

* tag.tag_name: 返回标签名称
* tag.tag_href: 返回标签链接
* tag.tag_count: 返回标签下的文章数目
...
<article class="post">
    <div class="post-head">
        ...
        <div class="post-meta">
            ...
            <time class="post-date"><%=helper.fns.dateFormat(new Date(item.art_post_time), 'yy年m月d日')%></time>
        </div>
    </div>
...
* article.art_title: 返回文章标题
* article.art_cover: 返回文章封面图片
* article.art_href: 返回文章链接地址

#### 插件页面模板文件

插件页面模板文件不是一个特定的文件，其文件名由插件规定，具体参考插件的说明文档。
...
exports.getRndArticle = function() {
    ...
    blog.modules.db
        ...
        .forEach(function(article) {
            article.art_title = fns.cutStr(article.art_title, 35, true);
...
exports.getHotArticle = function() {
    ...
    blog.modules.db
        .runsql(sql)
        .forEach(function(article) {
            article.art_title = fns.cutStr(article.art_title, 28, true);
